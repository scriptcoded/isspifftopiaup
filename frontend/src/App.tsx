...
async function fetchStatus(serverName: string): Promise<ServerInfo> {
  const response = await fetch(
    `${import.meta.env.VITE_BACKEND_URL}/status/${serverName}`,
  );
...
function getStatusText(status: ServerStatus | undefined) {
  switch (status) {
    ...
    case ServerStatus.Offline:
      return (
        <>
          Spifftopia is currently <span className="text-red-400">offline.</span>
        </>
      );
    ...
    <main className="min-h-screen flex justify-center items-center p-8">
      <div className="space-y-2 md:space-y-4 lg:space-y-6">
        {query.isLoading || query.isError ? (
          ...
          <>
            <h1 className="text-xl md:text-3xl lg:text-5xl">{statusText}</h1>
            {query.data?.status === ServerStatus.Online && (
              <h2 className="text-sm md:text-lg lg:text-2xl">
                There are{' '}
                <span className="text-sky-400">{query.data?.queue}</span> in
                queue and the estimated queue time is{' '}
            ...
            <p className="text-xs md:text-sm lg:text-base text-stone-400">
